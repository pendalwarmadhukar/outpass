...
const dir = 'c:\\Users\\madhu\\Desktop\\outpass\\frontend\\src\\styles';
...
processDir(dir);
// Also process App.css in src
const appCss = 'c:\\Users\\madhu\\Desktop\\outpass\\frontend\\src\\App.css';
if (fs.existsSync(appCss)) {
    let content = fs.readFileSync(appCss, 'utf8');
    let changed = false;
    replacements.forEach(r => {
        if (r.old.test(content)) {
            content = content.replace(r.old, r.new);
    ...
    if (changed) {
        fs.writeFileSync(appCss, content);
        console.log(`Updated: ${appCss}`);
    }
}
